...
class Aauth {
	...
	public function login($identifier, $pass, $remember = FALSE, $totp_code = NULL) {
		...
		// to find hrm_user id, create sessions and cookies
		$query = $this->aauth_db->where($db_identifier, $identifier);
		$query = $this->aauth_db->get($this->config_vars['users']);
	...
	//tested
	/**
	 * Check hrm_user login
	 * Checks if hrm_user logged in, also checks remember.
	 * @return bool
	 */
	public function is_loggedin() {
		...
		if ( $this->CI->session->userdata('loggedin') ){ 
			...
			if( ! $this->CI->input->cookie('hrm_user', TRUE) ){
				return FALSE;
			} else {
				$cookie = explode('-', $this->CI->input->cookie('hrm_user', TRUE));
	...
	/**
	 * Controls if a logged or public hrm_user has permission
	 * 
	 * If hrm_user does not have permission to access page, it stops script and gives
	 * error message, unless 'no_permission' value is set in config.  If 'no_permission' is
	 * set in config it redirects hrm_user to the set url and passes the 'no_access' error message.
	 * It also updates last activity every time function called.
	 *
	 * @param bool $perm_par If not given just control hrm_user logged in or not
	 */
	...
	/**
	 * Logout hrm_user
	 * Destroys the CodeIgniter session and remove cookies to log out hrm_user.
	...
	/**
	 * Remind password
	 * Emails hrm_user with link to reset password
	...
	/**
	 * Reset password
	 * Generate new password and email it to the hrm_user
	...
	/**
	 * Update last login
	 * Update hrm_user's last login date
	 * @param int|bool $user_id User id to update or FALSE for current hrm_user
	...
	/**
	 * Update remember
	 * Update amount of time a hrm_user is remembered for
	...
	/**
	 * Create hrm_user
	 * Creates a new hrm_user
	 * @param string $email User's email address
	 * @param string $pass User's password
	 * @param string $username User's username
	 * @return int|bool False if create fails or returns hrm_user id if successful
	 */
	...
	/**
	 * Update hrm_user
	 * Updates existing hrm_user details
	 * @param int $user_id User id to update
	...
	/**
	 * Get hrm_user
	 * Get hrm_user information
	 * @param int|bool $user_id User id to get or FALSE for current hrm_user
	...
	/**
	 * Verify hrm_user
	 * Activates hrm_user account based on verification code
	 ...
	 * Sends a verification email based on hrm_user id
	...
	/**
	 * Delete hrm_user
	 * Delete a hrm_user from database. WARNING Can't be undone
	...
	//tested
	/**
	 * Check hrm_user banned
	 * Checks if a hrm_user is banned
	 ...
	 * Check if hrm_user exist by username
	 * @param $user_id
	 *
	 * @return bool
	 */
	 ...
	 * Check if hrm_user exist by name
	 * @param $user_id
	 *
	 * @return bool
	 */
	...
	/**
	 * user_exist_by_email
	 * Check if hrm_user exist by hrm_user email
	 * @param $user_email
	 *
	 * @return bool
	 */
	...
	/**
	 * Get hrm_user id
	 * Get hrm_user id from email address, if par. not given, return current hrm_user's id
	 * @param string|bool $email Email address for hrm_user
	 * @return int User id
	 */
	...
	/**
	 * Update activity
	 * Update hrm_user's last activity date
	 * @param int|bool $user_id User id to update or FALSE for current hrm_user
	 * @return bool Update fails/succeeds
	 */
	...
	/**
	 * Add member
	 * Add a hrm_user to a group
	 * @param int $user_id User id to add to group
	 * @param int|string $group_par Group id or name to add hrm_user to
	...
	/**
	 * Remove member
	 * Remove a hrm_user from a group
	 * @param int $user_id User id to remove from group
	 * @param int|string $group_par Group id or name to remove hrm_user from
	...
	/**
	 * Remove member
	 * Remove a hrm_user from all groups
	...
	/**
	 * Is member
	 * Check if current hrm_user is a member of a group
	 * @param int|string $group_par Group id or name to check
	 * @param int|bool $user_id User id, if not given current hrm_user
	 * @return bool
	 */
	public function is_member( $group_par, $user_id = FALSE ) {

		// if user_id FALSE (not given), current hrm_user
		if( ! $user_id){
			$user_id = $this->CI->session->userdata('id');
		}
	...
	/**
	 * Is admin
	 * Check if current hrm_user is a member of the admin group
	 * @param int $user_id User id to check, if it is not given checks current hrm_user
	 * @return bool
	 */
	...
	/**
	 * Is hrm_user allowed
	 * Check if hrm_user allowed to do specified action, admin always allowed
	 * first checks hrm_user permissions then check group permissions
	 * @param int $perm_par Permission id or name to check
	 * @param int|bool $user_id User id to check, or if FALSE checks current hrm_user
	 * @return bool
	 */
	 ...
	 * @param int $perm_par Permission id or name to check
	 * @param int|string|bool $group_par Group id or name to check, or if FALSE checks all hrm_user groups
	 * @return bool
	 */
	public function is_group_allowed($perm_par, $group_par=FALSE){
		...
		// checks current hrm_user's all groups
		else {
	...
	/**
	 * Deny User
	 * Remove hrm_user from permission
	 * @param int $user_id User id to deny
	 ...
	 * Send a private message to another hrm_user
	 ...
	 * If receiver id not given retruns current hrm_user's pms, if sender_id given, it returns only pms from given sender
	 ...
	 * @param int|bool $receiver_id User id for message receiver, if FALSE returns for current hrm_user
	 ...
	 * @param string $key
	 * @param string $value
	 * @param int $user_id ; if not given current hrm_user
	 * @return bool
	 */
	public function set_user_var( $key, $value, $user_id = FALSE ) {
		...
		// if specified hrm_user is not found
    ...
    /**
	 * Get User Variables by hrm_user id
	 * Return array with all hrm_user keys & variables
	 * @param int $user_id ; if not given current hrm_user
	 ...
	 * @param int $user_id ; if not given current hrm_user
	 * @return bool|array, FALSE if no hrm_user vars, otherwise array
	 */
 ...
 * tamam //	 ismember la is admine 2. parametre olarak hrm_user id ekle
 * tamam // kepp infos errors die bişey yap ajax requestlerinde silinir errorlar
 * tmam // hrm_user variables
